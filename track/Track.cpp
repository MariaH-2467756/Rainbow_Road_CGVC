...
void Track::build(float width) {
  ...
  for (int i = 0; i < (int)m_points.size(); i++) {
    ...
    if (i > 0) {
      arcLength =
          m_arcLength.back() + glm::length(m_points[i] - m_points[i - 1]);
    }

    m_vertices.push_back(
        {m_points[i] + side * width, normal, {arcLength, 1.f}});
    m_vertices.push_back(
        {m_points[i] - side * width, normal, {arcLength, 0.f}});

    if (i > 0) {
      m_arcLength.push_back(m_arcLength.back() +
...
glm::mat4 Track::getTransformAtDistance(float d) const {
  ...
  if (hi == 0)
    hi = 1;
  if (hi >= (int)m_arcLength.size())
    hi = (int)m_arcLength.size() - 1;
  int lo = hi - 1;
