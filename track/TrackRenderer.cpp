...
void TrackRenderer::upload(const Track &ribbon, const char *textureFilePath) {
  ...
  glBufferData(GL_ARRAY_BUFFER, m_vertexCount * sizeof(TrackVertex),
               ribbon.vertices().data(), GL_STATIC_DRAW);
  ...
  // texCoord location 2
  glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(TrackVertex),
                        (void *)offsetof(TrackVertex, texCoord));
