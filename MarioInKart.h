...
class MarioInKart{
public:
    MarioInKart(): m_mario(ObjLoader().load(MARIO_OBJ_PATH, MARIO_TEXTURE_PATH)), m_kart(ObjLoader().load(KART_OBJ_PATH, KART_TEXTURE_PATH)), m_wheels(ObjLoader().load(WHEELS_OBJ_PATH, WHEELS_TEXTURE_PATH)) {};

    void draw(Shader& shader, const glm::mat4& model, const glm::mat4& view, const glm::mat4& projection){

        shader.useProgram();

        glm::mat4 kartModel = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
