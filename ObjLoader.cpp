...
MeshData ObjLoader::load(const std::string &objectFilepath,
  ...
  if (!reader.ParseFromFile(objectFilepath, config))
    throw std::runtime_error("ObjLoader: failed to load '" + objectFilepath +
                             "': " + reader.Error());
    ...
    for (const auto &idx : shape.mesh.indices) {
      ...
      if (vertices.find(key) == vertices.end()) {
        ...
        if (idx.texcoord_index >= 0) {
          v.texCoords = {attrib.texcoords[2 * idx.texcoord_index + 0],
...
void loadTexture(const char *textureFilePath, unsigned int &texture,
  ...
  if (strlen(textureFilePath) == 0) {
    ...
    unsigned char *data =
        stbi_load(textureFilePath, &width, &height, &nrChannels, 0);
    if (data) {
      GLenum format = (nrChannels == 4)
                          ? GL_RGBA
                          : GL_RGB; // Don;t use alhpa in shaders yet, but look
                                    // into updating that.
